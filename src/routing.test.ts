...
assert.match(mainSource, /preview\.addEventListener\('load', markLoaded/)
assert.match(mainSource, /feature-modal-close-label">Close</)
assert.doesNotMatch(mainSource, /<code>public\/images\/features\//)
assert.match(mainSource, /Failed to initialize page interactions/)
assert.match(mainSource, /void setupGitHubStars\(\)/)
assert.ok(mainSource.indexOf('${renderFooter()}\n    ${renderFeatureModal()}') > mainSource.indexOf('function renderHomePage()'))

const stylesheet = readFileSync('src/style.css', 'utf8')
assert.match(stylesheet, /\.feature-media-spinner \{[\s\S]*?animation: feature-media-spin/)
assert.match(stylesheet, /\.feature-modal-close \{[\s\S]*?top: 16px/)
assert.match(stylesheet, /\.feature-modal-close \{[\s\S]*?right: 16px/)
assert.match(stylesheet, /@media \(max-width: 803px\) \{[\s\S]*?\.feature-modal-close \{[\s\S]*?position: fixed/)
assert.match(stylesheet, /@media \(max-width: 803px\) \{[\s\S]*?\.feature-modal-close \{[\s\S]*?bottom: max\(20px, env\(safe-area-inset-bottom\)\)/)
assert.match(stylesheet, /\.feature-modal-close-label \{[\s\S]*?display: inline/)
assert.match(stylesheet, /@media \(max-width: 640px\) \{[\s\S]*?\.footer-blueprint-mark \{[\s\S]*?font-size: clamp\(42px, 13vw, 52px\)/)
